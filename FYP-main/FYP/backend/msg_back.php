...
if(!empty($message)){
    $sql = "INSERT INTO msg_db (sender_id , receiver_id , msg , file) 
    VALUES ('{$senderID}' , '{$receiverID}' ,'{$message}' ,'{$file_name}') ";
    mysqli_query($dbconnection , $sql);
} else {
    $sql = "INSERT INTO msg_db (sender_id , receiver_id , file) 
    VALUES ('{$senderID}' , '{$receiverID}' ,'{$file_name}') ";
    mysqli_query($dbconnection , $sql);
}
